    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-4 sm:px-8">
          ...
          {/* Always rendered (and left in the flex layout) so the nav links don't jump when this
              is hidden on /create and /profile — only visibility toggles, never the flex child count. */}
          <div className={`hidden lg:block ${hideCreateCta ? "invisible" : ""}`} aria-hidden={hideCreateCta}>
            <button
              onClick={() => navigate("/create")}
              tabIndex={hideCreateCta ? -1 : undefined}
        ...
        {mobileOpen && (
          <div className="animate-fade-in border-t border-[#241f1a]/10 bg-[#faf4ea] px-6 py-6 lg:hidden">
            <div className="flex flex-col gap-5">
              ...
              <NavLink to="/profile" onClick={() => setMobileOpen(false)} className="font-display text-lg text-[#17151a]">
                My Creations
              </NavLink>
              <NavLink to="/about" onClick={() => setMobileOpen(false)} className="font-display text-lg text-[#17151a]">
                About
              </NavLink>
            </div>
          </div>
      ...
      >
        <NavLink to="/marketplace" className="flex flex-col items-center gap-1 px-4 py-1">
          {({ isActive }) => (
            <>
              <IconStore className={`h-5 w-5 ${isActive ? "text-[#17151a]" : "text-[#17151a]/40"}`} />
              <span className={`text-[10px] uppercase tracking-wide ${isActive ? "text-[#17151a]" : "text-[#17151a]/40"}`}>Marketplace</span>
            </>
          )}
        </NavLink>
        ...
        <NavLink to="/profile" className="flex flex-col items-center gap-1 px-4 py-1">
          {({ isActive }) => (
            <>
              <IconUser className={`h-5 w-5 ${isActive ? "text-[#17151a]" : "text-[#17151a]/40"}`} />
              <span className={`text-[10px] uppercase tracking-wide ${isActive ? "text-[#17151a]" : "text-[#17151a]/40"}`}>Profile</span>
            </>
          )}
        </NavLink>
      </nav>
