...
export default function DrawLayer({ strokes, interactive, tool, color, eraseColor, onStrokeEnd }: DrawLayerProps) {
  ...
  return (
    <g>
      {strokes.map((s) => (
        <path key={s.id} d={s.d} stroke={s.color} strokeWidth={s.width} strokeOpacity={s.opacity} strokeLinecap="round" strokeLinejoin="round" fill="none" />
      ))}
